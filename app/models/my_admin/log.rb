class MyAdmin::Log < ActiveRecord::Base
  ...
  scope :my_admin_order_user, lambda { |params|
    joins(:user).
    order("my_admin_users.first_name #{params[:order]}, my_admin_users.last_name #{params[:order]}") if params[:order].present?
  }
  ...
  def first_name
     user = MyAdmin::User.find(self.user_id)
     user.first_name
  end
